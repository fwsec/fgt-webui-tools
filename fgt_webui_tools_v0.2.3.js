...
javascript: (async function () {
    ...
    function loadPrism() {
      ...
      const keywords = document.createElement("script");
      keywords.innerHTML = "\"undefined\"!=typeof Prism&&Prism.hooks.add(\"wrap\",(function(e){\"keyword\"===e.type&&e.classes.push(\"keyword-\"+e.content)}));"
    ...
    const prismKeywords = document.createElement("style");
    // Add keyword specific styling here
    prismKeywords.innerHTML = ".keyword.keyword-set { color: #f92672; }";
    document.head.appendChild(prismKeywords);
    ...
    function handleMessage(msg,it){
      ...
      if (!tableEl) {
        ...
        switch (it) {
          case "sys_cmd":tableEl.innerHTML = '<tr><th style="border:1px solid grey;">Key</th><th style="border:1px solid grey;">Value</th></tr>';break;
          case "arp_cmd1":tableEl.innerHTML = '<tr><th style="border:1px solid grey;text-align:left">IP Address</th><th style="border:1px solid grey">Age(min)</th><th style="border:1px solid grey">MAC Address</th><th style="border:1px solid grey">Interface</th></tr>';break;
          case "arp_cmd2":tableEl.innerHTML = '<tr><th style="border:1px solid grey">Interface</th><th style="border:1px solid grey;text-align:left">IP Address</th><th style="border:1px solid grey">MAC Address</th><th style="border:1px solid grey;text-align:left">Status</th><th style="border:1px solid grey">Use</th><th style="border:1px solid grey">Confirm</th><th style="border:1px solid grey">Update</th><th style="border:1px solid grey">Ref</th></tr>';break;
          default:tableEl.innerHTML = '';
        }
      ...
      switch (it) {
        case "sys_cmd":
          let [key, value] = msg.split(/:(.*)/s);
          value = value.replace(/[dD]isable/g,'<f-icon class="fa-disabled"></f-icon>&nbsp;<span style="color:silver">Disabled</span>');
          value = value.replace(/[nN]ot [aV]vailable/g,'<f-icon class="fa-disabled"></f-icon>&nbsp;<span style="color:silver">Not available</span>');
          value = value.replace(/[eE]nable/g,'<f-icon class="fa-enabled"></f-icon>&nbsp;<span>Enabled</span>');
          value = value.replace(/[cC]ertified/g,'<f-icon class="fa-enabled"></f-icon>&nbsp;<span>Certified</span>');
          tableEl.innerHTML += '<tr><td style="border: 1px solid grey;">'+key+'</td><td style="border: 1px solid grey;">'+value+'</td></tr>';
          break;
        case "arp_cmd1":
          const [v1, v2, v3, v4] = msg.replace(/\s+/g, " ").split(" ");
          if (v3.includes(':')){tableEl.innerHTML += '<tr><td style="border:1px solid grey">'+v1+'</td><td style="border:1px solid grey;text-align:center">'+v2+'</td><td style="border:1px solid grey;text-align:center">'+v3+'</td><td style="border:1px solid grey;text-align:center">'+v4+'</td></tr>'}
          break;
        case "arp_cmd2":
          let [w1, w2, w3, w4, w5, w6, w7, w8, w9] = msg.replace(/\s+/g, " ").split(" ");
          w5 = w5.replace("00000000",'<span title="Pseudo state used while an ARP entry is initially created or just before it is removed"><f-icon class="fa-connected"></f-icon>&nbsp;NONE<span>');
          w5 = w5.replace("00000001",'<span title="First ARP request sent"><f-icon class="fa-connected"></f-icon>&nbsp;INCOMPLETE</span>');
          w5 = w5.replace("00000002",'<span title="ARP response is received"><f-icon class="fa-connected"></f-icon>&nbsp;REACHABLE</span>');
          w5 = w5.replace("00000004",'<span title="ARP response is not received within expected time"><f-icon class="fa-custom-action"></f-icon>&nbsp;STALE</span>');
          w5 = w5.replace("00000008",'<span title="Schedule ARP request"><f-icon class="fa-clock-plus"></f-icon>&nbsp;DELAY</span>');
          w5 = w5.replace("00000010",'<span title="Actively sending ARP requests to try and resolve the address"><f-icon class="fa-clock-plus"></f-icon>&nbsp;PROBE</span>');
          w5 = w5.replace("00000020",'<span title="Not managed to resolve ARP within max configured number of probes"><f-icon class="fa-cancel"></f-icon>&nbsp;FAILED</span>');
          w5 = w5.replace("00000040",'<span title="Device does not support ARP e.g. IPsec Interface"><f-icon class="fa-dismiss"></f-icon>&nbsp;NOARP</span>');
          w5 = w5.replace("00000080",'<span title="Statically configured ARP entry">PERMANENT</span>');
          tableEl.innerHTML += '<tr><td style="border:1px solid grey;text-align:center">'+w2.substring(w2.indexOf("=")+1)+'</td><td style="border:1px solid grey">'+w3+'</td><td style="border:1px solid grey;text-align:center">'+w4+'</td><td style="border:1px solid grey">'+w5.substring(w5.indexOf("=")+1)+'</td><td style="border:1px solid grey;text-align:center">'+w6.substring(w6.indexOf("=")+1)+'</td><td style="border:1px solid grey;text-align:center">'+w7.substring(w7.indexOf("=")+1)+'</td><td style="border:1px solid grey;text-align:center">'+w8.substring(w8.indexOf("=")+1)+'</td><td style="border:1px solid grey;text-align:center">'+w9.substring(w9.indexOf("=")+1)+'</td></tr>';
          break;
        default:tableEl.innerHTML += msg+'\n';
      }
      Prism.highlightAll();
    }
    ...
    async function onMessage(evt) {
      let msg = await evt.data.text(); // convert incoming binary data to utf8 text
  
      if (!ready) {
        ready = true;
        sendCommand(command);
      } else {
        let lastChar = ""; // keep track of the last iterated character in the loop
        for (const i in msg) {
          const char = msg[i];
          ...
          if (lastChar === "\r") {
            ...
            message += char; // otherwise, just append to the message we're putting together
          }
    ...
    n.id = "tools";
    n.innerHTML = '<div id="menu-toggle" style="color:#000;background-color:goldenrod;margin:0;padding:4px 4px 1px 5px;border-left:3px solid #ffd700;display:flex;"><f-icon class="ftnt-webhook icon-lg"></f-icon>&nbsp;Tools<nu-icon class="icon" style="margin-left:auto;width:18px;height:18px;transform:rotate(90deg)">'+arrosvg+'</div>';
    ...
    items.forEach((item, i) => {
      ...
      itemEl.addEventListener("mousedown", (event) => {event.preventDefault()}); // Prevent text selection
