/*
FortiGate WebUI Tools v0.2.6 (Fortinet Community Edition)
...
javascript: (async function () {
  ...
  function formatStatusWithText(status) {
    if (status == null)
      return "";

    return status ?
      '<f-icon class="fa-status-up"></f-icon> Up' :
  ...
  function formatBytes(bytes) {
    if (bytes >= 1_000_000_000_000)
      return "".concat(Math.round(bytes / 1_000_000_000_000), " TB");

    if (bytes >= 1_000_000_000)
      return "".concat(Math.round(bytes / 1_000_000_000), " GB");

    if (bytes >= 1_000_000)
      return "".concat(Math.round(bytes / 1_000_000), " MB");

    if (bytes >= 1_000)
      return "".concat(Math.round(bytes / 1_000), " KB");

    return "".concat(bytes ?? 0, " B");
  }
  ...
  function formatRelativeTime(time) {
    ...
    return "".concat(time ?? 0, " seconds");
  }

  function formatProxiesStatus(proxies) {
    if (proxies.every(p => p.status == "up"))
      return '<f-icon class="fa-status-up"></f-icon> Up';

    if (proxies.every(p => p.status == "down"))
      return '<f-icon class="fa-status-down"></f-icon> Down';

    return "".concat(
      '<f-icon class="fa-status-up"></f-icon>',
      ' ',
      proxies.filter(p => p.status == "up").length,
      ' ',
      'Up',
      ' ',
      '<f-icon class="fa-status-down"></f-icon>',
      ' ',
      proxies.filter(p => p.status == "down").length,
      ' ',
      'Down'
    );
  }

  function formatProxiesDetail(name, proxies) {
    return proxies.map(q =>
      "".concat(
        proxies.length > 1 ?
            "".concat(
                "<b>",
        ...
        "<br><br>",
        "<b>Status</b>: ",
        formatStatusWithText(q.status == "up"),
        " → ",
        q.status == "up" ?
          '<a href="javascript:" data-action="tunnel_down" data-p1name="' + name + '" data-p2name="' + q.p2name + '" data-p2serial="' + q.p2serial + '">Bring Down</a>' :
          '<a href="javascript:" data-action="tunnel_up" data-p1name="' + name + '" data-p2name="' + q.p2name + '" data-p2serial="' + q.p2serial + '">Bring Up</a>',
      )
    ).join("<hr>");
  }

  function formatProxies(name, proxies) {
    return "".concat(
      formatProxiesStatus(proxies),
      ' ',
      '<a href="javascript:" data-action="expand_proxies">+ More</a>',
      '<div style="display: none;">',
      '<hr>',
      formatProxiesDetail(name, proxies),
      '</div>'
    );
  }
  ...
  function getToken() {
    const token = document.cookie
      .split("; ")
      .find((row) => row.startsWith("ccsrftoken"))
      ?.split("=")[1];

    return JSON.parse(token);
  }

  function showStatus(method, response) {
  ...
  async function changeTunnelStatus(p1name, p2name, p2serial, status) {
    fetch("/api/v2/monitor/vpn/ipsec/tunnel_" + status, {
      method: "POST",
      headers: {
  ...
  async function resetStats(p1name) {
    fetch("/api/v2/monitor/vpn/ipsec/tunnel_reset_stats", {
      method: "POST",
      headers: {
        "Content-Type": "application/json;charset=UTF-8",
        "X-Csrftoken": getToken()
  ...
  async function clearIke(p1name) {
    fetch("/api/v2/monitor/vpn/ike/clear", {
      method: "POST",
      headers: {
        "Content-Type": "application/json;charset=UTF-8",
        "X-Csrftoken": getToken()
  ...
  function documentClick(event) {
    ...
    switch (actionElement.dataset.action) {
      ...
      case "tunnel_up":
        changeTunnelStatus(actionElement.dataset.p1name, actionElement.dataset.p2name, +actionElement.dataset.p2serial, "up");
        break;

      case "tunnel_down":
        changeTunnelStatus(actionElement.dataset.p1name, actionElement.dataset.p2name, +actionElement.dataset.p2serial, "down");
        break;
      ...
      case "reset_stats":
        resetStats(actionElement.dataset.p1name);
        break;
  ...
  fetch('https://api.github.com/repos/fwsec/fgt-webui-tools/contents')
    ...
    function findDirectory() {
      ...
      const targetScript = Array.from(scripts).find((script) =>
        /\/([a-f0-9]{32}|static)\/runtime\.js/.test(script.src)
      );
      const key = targetScript.src.match(/\/([a-f0-9]{32}|static)\/runtime\.js/)[1];
      return key;
    }
    ...
    const ipsec_cmd = ["get vpn ipsec stats crypto"];
    ...
    function buildTable(message, headersArg=undefined, invisibleHeaders=undefined, tableEntryFunc, lineSplitter=" ", contentFunc, subheaders){
      ...
      function buildHeader(localHeaders, invisibleHeaders){
        if(localHeaders && localHeaders.length > 0) {
          ...
          return '<tr>' + localHeaders.map((d, i) => {
            let columnSpanTxt = "";
            if(subheaders){
              let subHeader = subheaders.find(subHeader => {
                const idx = d.indexOf(subHeader.parent)
                return idx >= 0;
              });
              if(subHeader) {
                columnSpanTxt = 'colspan="' + subHeader.headers.length+ '"';
              } else if(subheaders){
                columnSpanTxt = 'rowspan="2"'
              }
            }
            return '<th style="border:1px solid grey;text-align:center;"' + columnSpanTxt +'>'+d+'</th>';

          }).join('') + '</tr>'.concat(subheaderHtml)
        }
      }

      function beautifyData(data, tableEntryFunc){
        return data.map(entry => {
          ...
          if(tableEntryFunc) return tableEntryFunc(entry);
          return entry;
        })
      }

      function buildContent(message, localHeaders, tableEntryFunc, lineSplitter){
      ...
      let contentDOM = contentFunc ? contentFunc(message) : buildContent(message, localHeaders, tableEntryFunc, lineSplitter);
      return headerDOM+ '\r\n' + contentDOM;
    }

    // Function to find the index of the entry containing the specific string
    function findIndexOfSpecificString(array, searchString) {
      for (let i = 0; i < array.length; i++) {
        if (array[i].includes(searchString)) {
          return i;
        }
      }
      return -1; // Return -1 if the specific string is not found in any entry
    }

    let headers=[];
    function handleMessage(msg, it) {
      const tableMode = ["sys_cmd", "arp_cmd1", "arp_cmd2", "ipsec_tunnel"].includes(it);
      ...
      switch (it) {
        ...
        case "arp_cmd2":
          ...
          case "ntp_info":
            ...
            break;
        case "ipsec_tunnel":
          headers = ['Index', 'Device', 'Encryption (encrypted / decrypted)', 'Integrity (generated / validated)']
          subheaders = [
            { parent: "Encryption (encrypted / decrypted)", headers: ['null', 'des', '3des', 'aes-cbc', 'aes-gcm', 'aria', 'seed', 'chacha20poly1305'] },
            { parent: "Integrity (generated / validated)", headers: ['null', 'md5', 'sha1', 'sha256', 'sha386', 'sha512'] }
          ]
          invisibleHeaders= ['Index']
          msg = msg.replace('IPsec crypto devices in use:', '');
          const contentFunc = (data) => {
            // split by empty lines
            const tableRows = data.split(/\n\s*\n/).map(section => {
              const sectionLines = section.trim().split(/\r\n/);
              const device = sectionLines[0].replace(":", "");
      ...
      },
      {
        ...
        script: async function (parentEl) {
          ...
          const columnAliases = {
            ...
            "incoming_bytes": "Incoming Data",
            "outgoing_bytes": "Outgoing Data",
            "comments": "Comments",
            "tun_id": "Local Gateway",
          ...
          const run = async () => {
            ...
            const render = () => {
              const customizerHTML =
                "".concat(
                  '<div id="ipsec-customizer" style="display:none;padding:5px;position:absolute;top:75px;left:0;right:0;margin:30px 124px 30px 50px;background-color:white;border:1px solid #000;z-index:1">',
                  '<div style="display:flex;flex-wrap:wrap;gap:5px">',
                  '<button id="ipsec-customizer-ok">OK</button>',
                  '</div>',
                  '<div style="display:flex;flex-wrap:wrap;gap:5px">',
                  [...columnSet].map(c => {
                    return "".concat(
                      '<label>',
                      activeColumnSet.has(c) ?
                        '<input type="checkbox" name="ipsec-columns" value="' + c + '" checked>' :
                        '<input type="checkbox" name="ipsec-columns" value="' + c + '">',
                      columnAliases[c] ?? c,
                      '</label>'
                    )
                  }).join("")
              ...
              const tableHTML =
                "".concat(
                  '<button id="ipsec-customizer-trigger">Select&nbsp;&rsaquo;</button>',
                  '<button id="ipsec-customizer-default" ' + (activeColumnSet.size == defaultColumnSet.size && [...activeColumnSet].every((x) => defaultColumnSet.has(x)) ? 'disabled' : '') + '>Default</button>',
                  '<button id="ipsec-customizer-all" ' + (activeColumnSet.size == columnSet.size ? 'disabled' : '') +  '>All</button>',
                  '<button id="ipsec-customizer-refresh"><f-icon class="fa-refresh"></f-icon></button>',
                      ...
                      [...activeColumnSet].map(c => {
                        return "".concat(
                          '<td style="padding: 5px">',
                            ...
                            c == "creation_time" ?
                              "".concat(
                                formatStatusWithText(p["connection_count"] > 0),
                                p["connection_count"] > 0 ?
                                  "".concat(' (', formatRelativeTime(p[c]), ')') : ""
                              ) :
                            c == "proxyid" ?
                              formatProxies(p["name"], p[c]) :
                            typeof p[c] == "object" ?
                              JSON.stringify(p[c]) : p[c],
                          '</td>'
                        )
                      }).join(""),
                      '<td style="padding: 5px; text-align: center;">',
              ...
              p.addEventListener("click", documentClick);

              document.getElementById("ipsec-customizer-trigger").addEventListener("click", () => {
                document.getElementById("ipsec-customizer").style.display = "block";
              });

              document.getElementById("ipsec-customizer-ok").addEventListener("click", () => {
                const ces = document.getElementsByName("ipsec-columns");
              ...
              document.getElementById("ipsec-customizer-default").addEventListener("click", () => {
                activeColumnSet.clear();
              ...
              document.getElementById("ipsec-customizer-refresh").addEventListener("click", () => {
                run();
              });

              const commandListEl = document.createElement("ul");
              commandListEl.id = "ipsec-tunnel-command-list";

              ipsec_cmd.forEach((command) => {
                const el = document.createElement("li");
                el.style.marginBottom = "1rem";
                el.style.color = "green";
                el.style.cursor = "pointer";
                ...
                el.onclick = function () {
                  ...
                  if (!tableEl) {
                    ...
                    active_command = command;
                    arrwoEl.style.transform = opened ? "rotate(90deg)" : "rotate(0deg)";
                  } else {
                    if (tableEl.dataset.active === "active") {
                      tableEl.dataset.active = "";
                      tableEl.style.display = "none";
                  ...
                  function toggleTableVisibility(table, show) {
                  ...
                  if(tableElement) {
                    toggleTableVisibility(tableElement, opened);
                  }
                };
                commandListEl.appendChild(el);
              });
              ...
              dataPromise.then(response => showStatus("GET", response));
            };
